...
//#endregion
//#region node_modules/.pnpm/marked@15.0.11/node_modules/marked/lib/marked.esm.js
/**
...
function escape(html, encode) {
	...
	} else if (other.escapeTestNoEncode.test(html)) return html.replace(other.escapeReplaceNoEncode, getEscapeReplacement);
	return html;
}
...
function rtrim(str, c, invert) {
	...
	if (l === 0) return "";
	let suffLen = 0;
	while (suffLen < l) if (str.charAt(l - suffLen - 1) === c && true) suffLen++;
	else break;
...
var _Tokenizer = class {
		...
		if (cap) {
			...
			for (let i = 0; i < list.items.length; i++) {
				...
				if (!list.loose) {
					...
					list.loose = spacers.length > 0 && spacers.some((t) => this.rules.other.anyLine.test(t.raw));
				}
			}
	...
	reflink(src, links) {
		...
		if ((cap = this.rules.inline.reflink.exec(src)) || (cap = this.rules.inline.nolink.exec(src))) {
			const link = links[(cap[2] || cap[1]).replace(this.rules.other.multipleSpaceGlobal, " ").toLowerCase()];
			if (!link) {
...
//# sourceMappingURL=vendor-markdown-Cwvpr7zF.js.map