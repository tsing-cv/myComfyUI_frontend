import { boolean, description, literal, minValue, number, optional, pipe, strictObject, union } from "valibot";
...
import { display, splitClasses } from "../utils/utils.js";
export const enforceConsistentLineWrapping = createRule({
    ...
    schema: strictObject({
        classesPerLine: optional(pipe(number(), minValue(0), description("The maximum amount of classes per line.")), 0),
        group: optional(pipe(union([
            literal("newLine"),
            literal("emptyLine"),
            literal("never")
        ]), description("Defines how different groups of classes should be separated.")), "newLine"),
        indent: optional(pipe(union([
            literal("tab"),
            pipe(number(), minValue(0))
        ]), description("Determines how the code should be indented.")), 2),
        lineBreakStyle: optional(pipe(union([literal("unix"),
            literal("windows")]), description("The line break style.")), "unix"),
        preferSingleLine: optional(pipe(boolean(), description("Prefer a single line for different variants.")), false),
        printWidth: optional(pipe(number(), minValue(0), description("The maximum line length before it gets wrapped.")), 80),
        strictness: optional(pipe(union([
            literal("strict"),
            literal("loose")
        ]), description("Enable this option if prettier is used in your project.")), "strict")
    }),
    initialize: ctx => {
    ...
    for (const literal of literals) {
        ...
        if (groupedClasses) {
            for (let g = 0; g < groupedClasses.length; g++) {
                ...
                if (isFirstGroup && (literal.isInterpolated && !literal.closingBraces ||
...
function groupClasses(classes, dissectedClasses) {
    if (classes.length === 0) {
        return;
    }
