...
const getEsLintRuleName = (rule, options = {}) => {
  if (!rule.includes("/")) {
    ...
    if (!options.withNursery && found2 in rulesByCategory.nurseryRules) {
      return void 0;
    }
  ...
  if (!options.withNursery && found in rulesByCategory.nurseryRules) {
    return void 0;
  }
...
const handleRulesScope = (oxlintRules, rules, options = {}) => {
  for (const rule in oxlintRules) {
    ...
    if (isActiveValue(oxlintRules[rule])) {
      ...
      if (!eslintName.includes("/") && constants.typescriptRulesExtendEslintRules.includes(eslintName)) {
      ...
      if (eslintName.startsWith("@typescript-eslint/")) {
        ...
        if (constants.typescriptRulesExtendEslintRules.includes(baseRule) && allRules.includes(baseRule)) {
          rules[baseRule] = "off";
        }
      }
